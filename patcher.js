'use strict';
// NAVEE firmware patcher: identify a stock .bin, apply the byte-table patches, reseal the
// header with a fresh CRC-16/XMODEM. A wrong or already-patched image is refused. Speed patches
// differ per BLDC family; the board-magic gate blocks cross-flashing between families.
...
// ---- The three approved NAVEE NT5 Max firmware images ----
// Each entry: how to recognise it, how to verify it is untouched stock, how to re-seal it,
// and which byte-table patches apply. Meter is shared by both variants; BLDC is split.
const IMAGES = {

  // Display/meter, byte-identical across 9301 and 9701. Carries kickstart + cruise.
  meter: {
    ...
    verify: { size: 0x24C00 /* EOF = bodyBase 0x400 + len 0x24800 = 150528 */, crcOff: 0x13, crcStock: 0xe7ab, lenOff: 0x10, lenStock: 0x024800 },
    bodyBase: 0x400,
    // reseal: body length as BE-24 @0x10, CRC-16/XMODEM over [0x400,EOF) as BE-16 @0x13.
    reseal: (u8) => {
      const eof = u8.length;
    ...
    patches: [
      // Kickstart / zero-start: bcs -> b, skip the <3->3 region clamp (regionless zero-start).
      { off: 0x14bf3, from: [0xd2], to: [0xe0], id: 'kickstart' },
      // Cruise / Tempomat: first beq store -> unconditional b store (stores cruise in every region).
  ...
  bldc9701: {
    ...
    kind: 'bldc',
    // 16-byte board magic at 0x90 uniquely identifies the family (blocks cross-flashing).
    ...
    reseal: (u8) => {
      ...
      // secondary field @0x13 ships as garbage on this family and is ignored by the loader.
      // leave it exactly as stock (do NOT recompute - that would deviate from a factory image).
    },
    patches: [
      { off: 0x37fe, from: [0xb9, 0xf5, 0xc3, 0x7f], to: [0xb9, 0xf5, 0xfe, 0x7f], id: 'speed-390a' }, // 39.0->50.8
      ...
      { off: 0x4f7c, from: [0xb4, 0xf5, 0xd2, 0x7f], to: [0xb4, 0xf5, 0x08, 0x7f], id: 'speed-420' },   // 42.0->54.4 margin
    ],
  },
...
// Identify which approved image this is, or null.
function identify(u8) {
  for (const key of Object.keys(IMAGES)) if (IMAGES[key].match(u8)) return key;
  return null;
}

// Verify the image is untouched stock (so a wrong or already-patched file is refused, not damaged).
...
// Main entry: take the stock .bin (ArrayBuffer), return the patched+resealed image.
// Throws on an unrecognised, wrong, or already-patched image (never returns a damaged file).
...
// Expose for the page and for tests (Node).
